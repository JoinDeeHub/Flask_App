pipeline {
  ...
  stages {
    ...
    stage('Package') {
      steps {
        // packaging with tolerance for transient file updates + retry
        script {
          def rc = 1
          def attempts = 0
          while (rc != 0 && attempts < 2) {
            ...
            rc = sh(script: '''
              ...
              # ignore file-changed warnings (tar 1.28+)
        ...
        withCredentials([sshUserPrivateKey(credentialsId: "${env.SSH_CRED_ID}",
          ...
          sh label: 'Upload and deploy to EC2', script: """
            ...
            ssh -o BatchMode=yes -o StrictHostKeyChecking=no -i \"${KEY}\" \"${REMOTE}\" bash -lc \"
              ...
              # restart systemd service if present; tolerate missing service gracefully
  ...
  post {
    ...
    failure {
      ...
      // optional: you can call emailext here if configured
    }
    cleanup {
